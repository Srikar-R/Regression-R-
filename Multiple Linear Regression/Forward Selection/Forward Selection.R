...
#It is one of two commonly used methods of stepwise regression; the other is backward elimination, and is almost opposite. In that, you start with a model that includes 
#every possible variable and eliminate the extraneous variables one by one.Forward selection typically begins with only an intercept. One tests the various variables that may be relevant, and the ‘best’ variable—where “best” is determined by some pre-determined 
#criteria—is added to the model
...
#As we previosly observed, the intercept is significant but has no true meaning as height, width and length of fish can't be 0
